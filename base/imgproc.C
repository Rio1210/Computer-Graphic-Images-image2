...
void ImgProc::value_in(double x, double y, std::vector<float> &pixel) const{
     pixel.clear();
     int i = int(x);
     int j = int(y);
   if( img_data == nullptr ){ return; }
...
void ImgProc::histogram(vector<vector<float>> &eq, vector<vector<float>> &CDF, vector<vector<float>> &PDF, vector<vector<float>> &his_pix)
{
